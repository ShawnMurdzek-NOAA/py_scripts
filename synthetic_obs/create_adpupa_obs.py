...
1. Start by extracting fields within a subdomain that's 500 km X 500 km centered on the radiosonde
    launch site (better yet, make the size of the domain an input parameter). This will allow us
    to hold multiple 3D fields in memory.

2. Try to determine radiosonde locations at each pressure level in the prepbufr. This will be a
    little tricky b/c the ascent rate and vertical velocities are in m/s, so we'd need to convert
    from m to Pa. These pressure levels are already relatively close together (I think), so taking 
    a (u, v, w) layer average should be fine to determine radiosonde drift.
