...
Jet - 4 GB of memory on tjet is sufficient
...
for i in range(ntimes):
    ...
    wrf_lat = ds['gridlat_0'].values
    wrf_lon = ds['gridlon_0'].values
    ds.close()
        ...
        if subset['subset'] in ['ADPSFC', 'SFCSHP', 'MSONET']:
            ...
            for o, m in zip(obs_name, wrf_name):
                if not np.isnan(subset[o]):
                    ...
                    out_df.loc[j, o] = (twgt * (xwgt1 * ywgt1 * wrf_ds[m][wrf_hr[ihr]][yi0, xi0] +
