...
upp_file = '/scratch1/BMC/wrfruc/murdzek/nature_run_spring_v2/output/202204291200/UPP/wrfnat_202204291300.grib2'
save_fname = 'upp_fields.pdf'

# Hybrid level fields (excluding the "_P0_L105_GLC0" suffix)
hyb_fields = ['TMP', 'SPFH', 'CLWMR', 'ICMR', 'RWMR', 'SNMR', 'GRLE', 'HAILMXR', 'SPNCR', 'SPNCS',
              'SPNCG', 'SPNCH', 'UGRD', 'VGRD', 'VVEL', 'PRES', 'HGT', 'NCONCD', 'NCCICE', 'FRACCC',
              'REFD', 'TKE']
hyb_z = [20] * len(hyb_fields)

# Isobaric fields (excluding the "_P0_L100_GLC0" suffix)
iso_fields = ['TMP', 'DPT', 'UGRD', 'VGRD', 'HGT']
iso_z = [2] * len(iso_fields)
...
# Create master lists
fields = hyb_fields + iso_fields + o3d_fields + flat_fields
zind = hyb_z + iso_z + o3d_z + [np.nan] * len(flat_fields)
